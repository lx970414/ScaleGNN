...
def load_dataset(name, device):
    ...
    if name == "ogbn-arxiv":
        ...
        t = perf_counter()
        adj_raw = None
        # adj_raw = g.adjacency_matrix().coalesce() #transpose=Tru
        # pre_time = perf_counter() - t
        # print("DGL to ADJ time: {:.4f}s".format(pre_time))
    elif name == "ogbn-mag":
    ...
    return g, adj_raw, features, labels, n_classes, train_nid, val_nid, test_nid, evaluator
